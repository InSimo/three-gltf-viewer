...
THREE.GLTFLoader = ( function () {
	...
	GLTFLoader.prototype = {
		...
		parse: function ( data, path, onLoad, onError ) {
			...
			if ( magic === BINARY_EXTENSION_HEADER_MAGIC ) {
                binaryBlob = new Blob([data], {type: 'model/gltf.binary'});
	...
	GLTFDracoMeshCompressionExtension.prototype.decodePrimitive = function ( primitive, parser ) {
		...
		for (var attributeName in attributesIdMap) {
			if (attributeName in this.glTFNameToThreeJSName) {
	...
	function resolveURL( url, path ) {
		...
		// Transform URL
		if ( window.gltfPathTransform !== undefined ) {

			return window.gltfPathTransform( url, path );
