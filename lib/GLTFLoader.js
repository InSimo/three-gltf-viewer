...
THREE.GLTFLoader = ( function () {
	...
	GLTFLoader.prototype = {
		...
		parse: function ( data, path, onLoad, onError ) {
			...
			// TODO: the standard also define a "minVersion" property that should be checked first
	...
	GLTFDracoMeshCompressionExtension.prototype.decodePrimitiveAccessors = function ( primitive, parser, accessors ) {
		...
		var compressedBufferViewIndex = primitive.extensions[ this.name ].bufferView;
		var dracoAttributesToAccessorId = {};
		var dracoAttributesToArrayType = {};
		// indices
		dracoAttributesToArrayType[-1] = WEBGL_COMPONENT_TYPES[jsonAccessors[primitive.indices].componentType];
		// attributes and morph targets
		var compressedAttributesIdMapArray = [ primitive.extensions[ this.name ].attributes ];
		var compressedTargetsArray = primitive.extensions[ this.name ].targets;
		var decodedAttributesIdMapArray = [ primitive.attributes ];
		var decodedTargetsArray = primitive.targets;
		if (compressedTargetsArray !== undefined) {
			compressedAttributesIdMapArray = compressedAttributesIdMapArray.concat(compressedTargetsArray);
			decodedAttributesIdMapArray = decodedAttributesIdMapArray.concat(decodedTargetsArray);
		}
		for (var i = 0, il = compressedAttributesIdMapArray.length; i < il; ++i) {
			var compressedAttributesIdMap = compressedAttributesIdMapArray[i];
			var decodedAttributesIdMap = decodedAttributesIdMapArray[i];
			for (var attributeName in compressedAttributesIdMap) {
				var dracoId = compressedAttributesIdMap[attributeName];
				var accessorId = decodedAttributesIdMap[attributeName];
				dracoAttributesToAccessorId[dracoId] = accessorId;
				dracoAttributesToArrayType[dracoId] = WEBGL_COMPONENT_TYPES[jsonAccessors[accessorId].componentType];
			}
		}
    ...
    GLTFParser.prototype.loadGeometries = function ( primitives ) {
		...
		return this._withDependencies( [
			...
			console.log(dependencies.accessors);
